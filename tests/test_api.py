...
def test_health_reports_ready(client):
    assert client.get("/api/health").json() == {"status": "ok", "pipeline_ready": True}
...
def test_image_ocr_text_runs_through_complete_pipeline(ocr_client):
    ...
    assert fake_ocr.unload_calls == 1
    assert fake_ocr.setup_calls == 1
    assert payload["input_text"].startswith("The seller offered")
    assert payload["ocr"]["model"] == "deepseek-ai/DeepSeek-OCR"
